...
class TemplateSettingsCommand: AbstractCommand() {
    @BaseCommand
    suspend fun run(
        event: SlashCommandInteractionEvent,
        ctx: InteractionContext
    ) {
        event.replyEmbeds(Embeds.default("You can configure templates in the ${Emojis.dashboard.formatted} [`Dashboard`](${Links.GUILD_DASHBOARD(ctx.guild.id)})"))
